object Program09  { def ques1() : Unit = {
var z: Array[String] = new Array[String](3) z(0) = "Aman"
z(1) = "Arun"
z(4/2) = "Karan"
...
def ques4() : Unit = {
val z = Array("Aman", "Arun", "Karan") z(0) = "Aman"
// z cannot be re-assigned but the Array elements are mutable
}

def ques5() : Unit = {
val z1 = Array("Aman", "Arun", "Karan")
 
val z2 = Array("Aman", "Arun", "Karan")
/*
z2=z1 gives:
z2=z1
^
...
val arr = Array.ofDim[Char](n,n) for(i<-0 until n)
for(j<-0 until n) if(j<=i)
arr(i)(j)='*' else
arr(i)(j)=' ' for(i<-0 until n)
{
 
for(j<-0 until n) print(arr(i)(j))
println()
}
}
...
def populate() : Unit = {
val arr = Array.range(1,20,3) println(s"\nArray: [${arr.mkString(" ")}]")
}

def concate(arr1: Array[Int],arr2: Array[Int]) : Unit ={ println(s"\nArray 1: [${arr1.mkString(" ")}]")
println(s"Array 2: [${arr2.mkString(" ")}]")
println(s"Concatenated Array: [${Array.concat(arr1,arr2).mkString(" ")}]")
}
 
def main(args: Array[String]) : Unit = { ques1()
ques2() ques3() ques4() ques5() pattern1(4) pattern2(6)
sortArray(Array(1, 10, 2, 4, 1, 2, 6, 4)) matrixPrint(Array(Array(1,2,3),Array(4,5,6),Array(7,8,9))) populate()
